...
class CandidateSkillController {
    ...
    public async readSkillById(req: Request, res: Response) {
        const jobId = parseInt(req.params.jobId);
        const candidateProfileId = parseInt(req.params.candidateProfileId);

        const candidateSkill = await candidateSkillService.readSkillById(candidateProfileId, jobId, req.currentUser);
